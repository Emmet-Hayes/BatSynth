...
public:
    ...
    void paint(Graphics& g) override {
        g.setColour(noteToColorMap[currentNoteColor]); //coloring the waveforms
  ...
  void process(const SampleType* data, size_t numSamples, float minFrequency) {
    ...
    if (state == State::collecting) {
      while (index++ < numSamples) {
        ...
        if (numCollected == buffer.size()) { //if we got to the end of the buffer
    ...
    // update the current note color based on minimum frequency
    // notify listeners of changes (like the actual GUI scope component)
